...
type: shaped-work
id: DGB-15
title: Annotate cleanup docs with forward-reference notice pointing to DGB-10
status: shaped
created: 2026-03-19
appetite: trivial
priority: P3
...
acceptance_criteria:
  - id: AC-1
    description: docs/cleanup/20260315_coupling_cohesion_pass1.md has a header notice explaining that remedy paths (src/domain/, src/infrastructure/, src/formatting.ts, etc.) reference DGB-10's planned target structure and do not yet exist
    status: pending
  - id: AC-2
    ...
    status: pending
  - id: AC-3
    description: Both notices include a link to the relevant backlog item (DGB-10 or DGB-4) so a reader can navigate to the execution plan
    status: pending
---

# DGB-15: Annotate cleanup docs with forward-reference notice pointing to DGB-10

## Problem

The DX Coach report flagged 21 "broken path references" across two cleanup analysis documents. On investigation, these are not broken references — they are intentional forward references to the target structure planned in DGB-10 (Ports & Adapters restructure) and DGB-4 (surface area reduction).
...
- `docs/cleanup/20260315_coupling_cohesion_pass1.md`: references `src/domain/`, `src/infrastructure/`, `src/formatting.ts`, `src/discord/webhookManager.ts`, `src/github/issueFormatter.ts` — all DGB-10 planned output
...
- **No-gos:** Rewriting remedy paths, executing DGB-10, deleting the cleanup docs
...
> [DGB-10](../backlog/DGB-10-ddd-ports-adapters-restructure.md) (Ports & Adapters
> restructure, status: shaped). These files do not yet exist. See DGB-10 for the
